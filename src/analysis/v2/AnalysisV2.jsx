...
//
// Drives the already-tested headless engine interactively via
// runFlow.deriveSession(history). Renders one question at a time (single- and
// multi-select), shows soft progress without assuming a fixed length, supports
// back/reset, and at completion renders the V2 Reveal (results experience) —
// it no longer auto-calls onComplete. The Reveal's CTA calls onContinue, which
// hands the legacy-bridged answers to the EXISTING completion flow.
//
// Canonical session contract (do not use nextId/done):
//   session.nextQuestion     the current question object (or null when complete)
//   session.nextQuestionId   the current question id    (or null when complete)
//   session.isComplete       true when the flow is finished
//   session.asked            ids answered so far
//   session.derived          derived V2 profile
//   session.archetype        { key, reasons }
//
// Mounted only when USE_ANALYSIS_V2 === true (see flag.js + App.jsx integration).
//
// Props:
//   onComplete(legacyAnswers, v2)  required — existing completion/handoff.
//   onExit()                       optional — "Start over" from the first screen.
//
// Style: Auro's existing T tokens (with safe fallbacks). Top-level function
// declarations per the project's React rule.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function AnalysisV2({ onComplete, onExit }) {
  // phase: 'landing' → 'flow' (reveal shows inside 'flow' once isComplete)
  ...
  function handoff() {
    onComplete(legacy, { archetype: session.archetype, derived: session.derived });
  }
  ...
  return (
    <div style={S.shell}>
      ...
      <div style={S.stage}>
        ...
        {phase === "flow" && !session.isComplete && currentQ && (
          <>
